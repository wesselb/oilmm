...
while True:
    try:
        ...
        total_mean = \
            parser.parse(Whitespace(), Literal('Mean:'), Whitespace(), Float())
        parser.find_line('Error:')
        total_error = \
            parser.parse(Whitespace(), Literal('Error:'), Whitespace(), Float())
...
for n in [100, 200, 300]:
    plt.plot(sorted(totals[n].keys()), get(totals[n], 0),
             '-o', label=f'$n={n}$')
plt.xlim(0, 250)
...
plt.subplot(1, 3, 2)
for n in [100, 200, 300]:
    plt.plot(sorted(hs[n].keys()), get(hs[n], 0) * 1e3,
             '-o', label=f'$n={n}$')
plt.xlim(0, 250)
plt.xlabel('Number of latent processes $m$')
...
plt.subplot(1, 3, 3)
for n in [100, 200, 300]:
    plt.plot(sorted(percs[n].keys()), get(percs[n], 0),
             '-o', label=f'$n={n}$')
plt.xlim(0, 250)
plt.xlabel('Number of latent processes $m$')
